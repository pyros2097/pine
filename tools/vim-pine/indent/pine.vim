...
setlocal indentexpr=GetPineIndent(v:lnum)
setlocal indentkeys=!^F,o,O,<:>,0),0],0},=elif

" Only define the function once.
if exists("*GetPineIndent")
  finish
endif
...
function! GetPineIndent(lnum)
  ...
  if cline =~ '^\s*\(elif\|else\)\>'
    return s:FindStartLine(a:lnum, '^\s*\(if\|elif\)')
  endif

  if pline =~ ':\s*$'
    return s:FindStartLine(plnum, '^\s*\(if\|else\|elif\|for\)\>') + &sw
  endif

  if pline =~ '=\s*$'
    return s:FindStartLine(plnum, '^\s*\(proc\|method\|test\|extern\)\>') + &sw
  endif

  " if we got here, this should be the begging of a multi-line if expression for example
  if pline =~ '^\s*\(if\|proc\|method\|test\|extern\|for\)[^:]*$'
    return plindent + &sw
  endif
  ...
  if pline =~ '^\s*\(break\|continue\|return\)\>'
